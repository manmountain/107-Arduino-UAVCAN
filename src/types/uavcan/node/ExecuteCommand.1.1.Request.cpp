...
size_t Request::serialize(uint8_t * payload) const
{
  ...
  if (uavcan_node_ExecuteCommand_Request_1_1_serialize_(&data, payload, &inout_buffer_size_bytes) < NUNAVUT_SUCCESS)
    return 0;
  else
    return inout_buffer_size_bytes;
}

void Request::operator = (Command const command)
{
  data.command = arduino::_107_::uavcan::to_integer(command);
}
